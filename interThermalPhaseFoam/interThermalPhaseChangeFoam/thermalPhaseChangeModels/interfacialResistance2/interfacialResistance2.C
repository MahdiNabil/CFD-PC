...
#include "interfacialResistance2.H"
...
{
    defineTypeNameAndDebug(interfacialResistance2, 0);
    addToRunTimeSelectionTable(thermalPhaseChangeModel, interfacialResistance2, dictionary);
}
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::thermalPhaseChangeModels::interfacialResistance2::interfacialResistance2
(
		const word& name,
...
void Foam::thermalPhaseChangeModels::interfacialResistance2::calcQ_pc()
{
...
Info << "dtUtilizedByTheThermalPhaseChangeModel = " << dT.value() << endl;
	...
	{
		threshold_[pI] = ( (alpha1_[pI] >= 0.0) && (alpha1_[pI] <= 1.0) ) ? 1.0 : 0.0; 
	}
...
bool Foam::thermalPhaseChangeModels::interfacialResistance2::read(const dictionary& thermalPhaseChangeProperties)
{
