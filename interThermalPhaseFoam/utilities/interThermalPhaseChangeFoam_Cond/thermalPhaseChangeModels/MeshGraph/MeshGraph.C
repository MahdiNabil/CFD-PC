...
F(Fin)
{
	...
	//Now allocate faces
	Faces.resize( Min.faceNeighbour().size() );	//This is the number of connected faces
	//Now populate faces
...
void MeshGraph::GetInterfaceCells( labelList& IntCells, const scalar& intVal)
{
	...
	for ( int i = 0; i < N; i++ )
	{
		...
		//Check where the interface lies relative to the cell face (on which side)
	...
	for ( std::set<label>::iterator it = IntCellsSet.begin(); it != IntCellsSet.end(); ++it )
	{  IntCells.append( *it );  }
}

//Like above, but returns cells on both sides of the interface...
void MeshGraph::GetDoubleInterfaceCells( labelList& IntCells, const scalar& intVal)
{
	//Traverse through graph faces
	int N = Faces.size();
...
//Another double layer method, but it actually returns the cells & data that are on the double layer:
void MeshGraph::GetInterfaceCellFacePairs( std::vector<CellFacePair>& IntCellFacePairs, const scalar& intVal)
{
	//Traverse through graph faces
	int N = Faces.size();
	...
	for ( int i = 0; i < N; i++ )
	{
		...
		//OK, now this cell face pair is an interface pair, so add it to the vector:
		//Populate struct - 
		CellFacePair curCFP;
		curCFP.f = i;
