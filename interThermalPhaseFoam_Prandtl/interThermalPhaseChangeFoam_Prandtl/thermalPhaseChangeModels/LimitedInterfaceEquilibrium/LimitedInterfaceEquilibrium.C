...
#include "LimitedInterfaceEquilibrium.H"
...
{
    defineTypeNameAndDebug(LimitedInterfaceEquilibrium, 0);
    addToRunTimeSelectionTable(thermalPhaseChangeModel, LimitedInterfaceEquilibrium, dictionary);
}
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::thermalPhaseChangeModels::LimitedInterfaceEquilibrium::LimitedInterfaceEquilibrium
(
		const word& name,
...
void Foam::thermalPhaseChangeModels::LimitedInterfaceEquilibrium::calcQ_pc()
{
...
//- This next section evaluates the phase change heat transfer on interface/wall cells
	...
	//Use limited alpha1:
    const volScalarField limAlpha1_( min(max(alpha1_, scalar(0)), scalar(1)) );

	//Get cond/evap limits
	volScalarField LimCond = (1.0-limAlpha1_)*( rho2*h_lv_ / dT );
	//No evaporation on wall cells!
	volScalarField LimEvap = (1.0-WallField)*limAlpha1_*rho1*h_lv_ / dT;
	//Apply fluid limiting
	volScalarField Q_pc_fluid = neg(Q_pc_org)*max(Q_pc_org, -LimCond) + pos(Q_pc_org)*min(Q_pc_org, LimEvap) ;
	
	//Time-based limiting (i.e. relative phase change rate can't exceed |1| per time step
	...
	volScalarField Q_pc_vol = Q_pc_org * mag( min( max(1.0/(PCV_fac+SMALL), -1.0), (1.0-WallField) ) );
...
bool Foam::thermalPhaseChangeModels::LimitedInterfaceEquilibrium::read(const dictionary& thermalPhaseChangeProperties)
{
